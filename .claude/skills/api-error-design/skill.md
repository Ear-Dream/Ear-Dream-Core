...
description: "API 에러 설계 패턴. 에러 코드 체계, 에러 응답 구조, 클라이언트 친화적 에러 메시지, 에러 카탈로그 구축, retry/fallback 전략을 제공하는 api-architect/backend-dev 확장 스킬. 'API 에러 설계', '에러 코드', '에러 응답', '에러 카탈로그', '에러 메시지', 'retry 전략' 등 API 에러 처리 체계 설계 시 사용한다."
---

# API Error Design — API 에러 설계 패턴

API 에러 체계, 메시지 설계, 복구 전략 레퍼런스.

## 대상 에이전트

원본 스킬은 `doc-writer` / `mock-tester`를 대상으로 했으나 **이 레포에는 없다.**

- `api-architect` — 에러 응답 형태 설계에 적용
- `backend-dev` — FastAPI 예외 처리와 pytest 시나리오에 적용
- `frontend-dev` / `api-integrator` — 에러 상태의 화면 표현에 적용

## ⚠️ 이 프로젝트 적용 (Ear Dream) — 아래가 이 문서의 일반 패턴보다 우선한다

**현재 에러 형식은 FastAPI 기본 `{"detail": ...}`이고 커스텀 핸들러가 없다.**
이 문서가 제안하는 계층적 에러 코드 체계·다국어 카탈로그를 도입하려면 **먼저 결정이 필요하다.**
지금 규모(엔드포인트 2개, 인증 없음)에 코드 체계를 먼저 세우는 것은 과하다.
필요해졌을 때 이 문서를 참고해 도입하고, 그 전까지는 기본 형식을 유지한다.

**이 서비스의 에러는 "요청이 틀렸다"보다 "인식이 안 됐다"가 압도적으로 많다.**
그리고 그 둘은 성격이 완전히 다르다:

| 상황 | HTTP | 성격 |
| --- | --- | --- |
| 윈도우 길이·점 개수가 계약과 다름 | 422 | **에러.** 조용히 받아서 추론하면 안 된다 |
| 랜드마크는 정상인데 아는 단어가 없음 | 200 | **에러가 아니다.** 빈 후보 목록은 정상 응답이다 |
| 손이 프레임을 벗어남 | 200 | 정상 응답 + 사유. 클라이언트가 안내 문구를 띄운다 |

**"인식 실패"를 4xx/5xx로 돌려주지 말 것.** 그러면 클라이언트가 재시도 로직을 태우게 되는데,
사용자가 해야 할 일은 재시도가 아니라 **동작을 다시 하는 것**이다.

**사용자 메시지 원칙 — 이 서비스에서는 특히 중요하다:**

- 사용자는 농인이고, 대화 도중에 이 화면을 본다. 상대방을 기다리게 한 채로 읽는다
- 소리 피드백에 의존할 수 없다. 실패는 반드시 시각적으로 드러나야 한다
- "다시 시도해주세요"로 끝내지 말 것. **무엇을 다시 해야 하는지**를 말한다 —
  동작을 다시 할지, 손을 화면 안에 넣을지, 잠시 기다릴지
- 기술 용어(랜드마크, 윈도우, 추론)를 사용자 메시지에 노출하지 않는다
