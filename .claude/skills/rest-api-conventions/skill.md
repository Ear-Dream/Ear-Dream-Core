...
## ⚠️ 이 프로젝트 적용 (Ear Dream) — 아래가 이 문서의 일반 컨벤션보다 우선한다

이 문서는 일반 REST 레퍼런스다. **Ear Dream에는 대부분 해당하지 않는다.**
핵심 엔드포인트가 리소스 CRUD가 아니라 **랜드마크 윈도우를 받아 단어를 돌려주는 추론 호출**이기 때문이다.

**이미 고정된 것 — 이 문서의 권고와 충돌하면 아래를 따른다:**

| 항목 | 이 문서의 권고 | Ear Dream 실제 | 판단 |
| --- | --- | --- | --- |
| 필드 네이밍 | (일반적으로 camelCase) | **snake_case** (`window_ms`) | Pydantic → 생성 TS까지 그대로 간다. 바꾸려면 생성 파이프라인 전체가 영향받으므로 명시적 결정 필요 |
| 에러 형식 | RFC 7807 Problem Details | FastAPI 기본 `{"detail": ...}` | 바꾸려면 별도 결정 항목으로 올린다. 임의로 7807을 도입하지 말 것 |
| 버전 | URL 버전 `/v1/` | `/api/v1` (`settings.api_v1_prefix`) | 이미 일치. `/health`만 루트 |
| 인증 | OAuth/JWT 설계 | **없음** | MVP 범위 밖 |

**이 서비스에 해당 없음 — 요구되지 않았다면 설계에 넣지 말 것:**

- **페이지네이션** (오프셋·커서 둘 다) — 목록 API는 `/phrases` 하나뿐이고 정적에 가깝다
- **HATEOAS** — 클라이언트가 `openapi-fetch`로 생성 타입을 쓰므로 링크 탐색이 필요 없다
- **응답 Envelope** — 생성 타입이 곧 계약이라 래핑은 중첩만 늘린다
- **Rate Limiting / Idempotency-Key** — 단일 기기 데모 단계다

**대신 이 프로젝트에서 실제로 중요한 것:**

- **비-CRUD 액션이 주인공이다.** `POST /recognize`처럼 동사형 엔드포인트가 예외가 아니라 기본이다.
  이걸 억지로 리소스 CRUD로 비틀지 말 것
- **요청 본문이 크고 구조적으로 모호하다.** 손·얼굴·포즈가 전부 `list[list[float]]`라
  위치로 나열하면 뒤바뀌어도 검증에 걸리지 않는다. 반드시 이름 있는 필드로 감싼다
- **422를 실제로 쓴다.** 윈도우 길이·점 개수가 계약과 다르면 조용히 받아서 추론하지 말고 거절한다.
  틀린 입력으로 나온 추론 결과는 디버깅이 거의 불가능하다
