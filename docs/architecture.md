...
| 로짓 편향 제거 | log-softmax에서 편향 벡터 차감 — 도메인 이동이 만든 특정 단어 과호출 억제 | 라벨 없는 실사용 아카이브로 추정 |
| reject 임계 | 최고 confidence 미달이면 `rejected` | 라벨된 라이브 클립 임계 곡선 |

⚠️ 뒤의 두 줄은 **현재 번들에서 꺼져 있다** (α=0 · 임계 0 = 거부 없음). 둘 다 추정에 쓴
모델의 출력 분포에 묶인 값이라 모델을 갈아끼우면 근거가 무효가 된다 — 편향 벡터를 다른
가중치에 쓰면 보정이 아니라 새 편향 주입이 되고, 현재 번들에는 캘리브레이션 산출물이
없다. 근거 없는 값을 남겨 두는 대신 꺼서 서빙한다.
...
| 모델 | top-1 | top-4 |
| --- | ---: | ---: |
| **Single-Observed-Hand (현재)** | **29.0%** | **54.3%** |
| SPOTER-208 | 26.1% | 41.3% |
| Hybrid H1b | 13.8% | 29.7% |

이 데이터의 왼손 검출률이 2.5%다 — 한 손으로 폰을 든 실제 자세이고, 현재 모델이
겨냥한 조건이다. 반대로 트리밍·결측 채움은 실측으로 유해했고(각각 −4%p, −1.5%p),
AR 보정을 끄면 붕괴한다(−9.5%p).
