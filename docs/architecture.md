...
라벨된 실사용 클립 138개(폰 세로 촬영, 학습·적응에 쓰이지 않은 사용자)로 잰 값이다.
스튜디오 수치와 혼동하지 말 것 — 같은 모델이 AI Hub test에서는 90% 안팎이다.

| 조건 | top-1 | top-4 |
| --- | ---: | ---: |
| **현재 서빙 (AR_TRAIN 9/16)** | **39.9%** | **69.6%** |
| AR_TRAIN 을 16/9 로 두면 | 29.0% | 54.3% |
| 얼굴 특징 제거 | 23.9% | 39.1% |
| 결측 손 채움 | 25.4% | 44.2% |
...
시간축 스무딩(창 7)은 +2.1%p였지만 AR 관례를 맞춘 뒤에는 이득이 대부분 사라져
채택하지 않았다. 손 검출률로 거르는 게이트도 같은 커버리지의 confidence 임계보다
약했다(31.4% vs 45.0%).
...
- **신규 사용자 일반화**: 도메인 적응에 쓴 3명과 다른 사용자에서는 성능이 크게 떨어진다
  (학습 레포 LOPO 평균 top-1 47.78%). 사용자별 적응 없이 쓰는 것이 현재 조건이다
