...
flowchart TB
  ...
  FEAT --> GATE["손 검출 마스크 게이팅"]
  GATE --> ENC["Transformer 인코더"]
...
**Hybrid H1b** — 수어 인식용 Transformer 인코더 분류기. 어휘 300단어(AI Hub 일상
고빈도 핵심단어), TorchScript로 서빙한다. 전신 SPOTER-208의 208차원 입력 계약과
300 클래스 인덱스를 그대로 이어받았고, 좌우 손이 **인코더를 공유**하도록 바꾼 뒤
한손/양손 보조 supervision을 얹어 학습했다 — 한 손만 보이는 입력이 취약했던
구조적 한계(아래)를 겨눈 변경이다.
...
- 손 검출 마스크: 프레임별 오른손·왼손 검출 여부. 미검출 손은 해당 구간이 0으로
  채워지는 대신 학습된 "결측" 벡터로 대체된다 — 0을 좌표로 오해하지 않게 하는 게이팅이다
- 출력: 서빙이 쓰는 것은 **300클래스 확률 하나**다. 모델에는 한손 106 분류기,
  한손/양손 이진 분류기, retrieval 임베딩이 함께 있지만 학습 레포 평가에서 300 head가
  가장 정확해 확정 분류에 쓰지 않기로 했고, 한손/양손 라우팅도 두지 않았다
...
⚠️ 뒤의 두 줄은 **현재 번들에서 꺼져 있다.** 편향 벡터와 임계는 둘 다 추정에 쓴 모델의
출력 분포에 묶인 값인데, 모델 교체로 그 근거가 무효가 됐다 — 편향 벡터는 다른 가중치에
쓰면 보정이 아니라 새 편향 주입이 되고, 임계는 캘리브레이션 산출물 자체가 아직 없다.
근거 없는 값을 남겨 두는 대신 α=0 · 임계 0(거부 없음)으로 서빙하고, 라벨된 라이브
평가셋을 확보한 뒤 temperature와 함께 다시 피팅한다.
...
- **한손 재조음 갭**: 어휘 300단어 중 194단어가 학습 데이터에서 양손 조음이다.
  ...
  인식이 급락한다. 현재 모델(Hybrid H1b)이 좌우 공유 인코더와 한손 supervision으로
  겨눈 지점이고 학습 레포 test에서는 개선이 확인됐지만, **실사용에서의 개선은
  미검증**이다 — 라벨된 라이브 평가셋이 없다
