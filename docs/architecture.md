...
**Single-Observed-Hand 300** — 수어 인식용 Transformer 인코더 분류기. 어휘 300단어
(AI Hub 일상 고빈도 핵심단어), TorchScript로 서빙한다. 208차원 입력 계약과 300 클래스
인덱스는 이전 세대들과 같고, 바뀐 것은 **손을 하나만 본다**는 점이다.

학습 데이터의 양손 단어도 대표 손 하나만 남기고 반대 손을 지운 채 학습했다. 그래서
서빙도 같은 형태로 넣는다 — 한 손으로 폰을 든 실사용 자세가 학습 분포 안에 들어온다.
추론 전에 그 단어가 한손 수어인지 양손 수어인지 알 필요가 없다.

여기에 실제 촬영 데이터로 **도메인 적응**을 얹었다. 배경·카메라·조명·사용자 차이를
줄이려고 직접 촬영분 CE에 AI Hub 혼합과 teacher distillation을 함께 걸었다.
...
- **대표 손 선택**: 검출률이 0.15 이상 차이 나면 많이 보인 손, 비슷하면 상하위 10%를
  잘라낸 이동량 중앙값이 큰 손. 선택되지 않은 손의 42차원은 0으로 지운다.
  ⚠️ 이 규칙은 학습 쪽과 **수치가 일치해야 한다** — 선택이 갈리면 모델이 보는 손이
  달라진다. 서버 구현과 학습 레퍼런스를 대조하는 테스트가 있다
...
- 손 검출 마스크: 프레임별 오른손·왼손 검출 여부. 0을 좌표로 오해하지 않게 하는 게이팅이다
- 출력: 서빙이 쓰는 것은 **300클래스 확률 하나**다. 한손/양손 이진 분류기와 retrieval
  임베딩이 함께 있지만 확정 분류에 쓰지 않는다 — 학습 레포가 한손/양손 hard routing을
  두지 않기로 결론냈다
- temperature scaling(번들 값) 후 임계 미달이면 `rejected`
...
정규화 좌표인가"가 모델 입력 계약의 일부**다. 서빙은 입력을 모델이 기대하는 관례로
사영한다.
...
⚠️ **`AR_TRAIN`은 모델마다 다르다.** 스튜디오 16:9만으로 학습한 모델은 16/9였지만, 현재
모델은 폰 세로 촬영본으로 도메인 적응을 해서 **세로 좌표를 그대로 기대한다**. 값이
어긋나면 에러 없이 정확도만 무너진다 — 실측으로 맞을 때 39.9%, 어긋날 때 29.0%였고,
반대로 같은 조작을 스튜디오 전용 모델에 걸면 23.2% → 7.2%로 붕괴한다. 즉 이 값은
취향이 아니라 **모델의 학습 좌표 관례**다.

⚠️ 뒤의 두 줄이 꺼진 이유는 둘 다 추정에 쓴 모델의 출력 분포에 묶인 값이기 때문이다.
편향 벡터를 다른 가중치에 쓰면 보정이 아니라 새 편향 주입이 되고, 현재 번들에는
캘리브레이션 산출물이 없다. 근거 없는 값을 남겨 두는 대신 꺼서 서빙한다.

라벨된 라이브 셋이 생긴 뒤로는 임계를 **잴 수 있다** — 다만 그 셋 하나로 임계를 고르고
같은 셋으로 성능을 보고하면 순환이라, 값을 박기 전에 별도 촬영분이 필요하다.
측정된 곡선은 「라이브 실측」 절에 있다.

### 라이브 실측

라벨된 실사용 클립 138개(폰 세로 촬영)로 잰 값이다.
스튜디오 수치와 혼동하지 말 것 — 같은 모델이 AI Hub test에서는 90% 안팎이다.

⚠️ **이 138클립은 현재 모델에게 "처음 보는 사람"이 아니다.** 이 사람의 다른 촬영 회차가
적응 학습에 들어가 있어서, 71.0%는 **등록된 사용자** 수치다. 처음 보는 사람 기대치는
학습 레포의 4-fold LOPO 평균 **54.19%**를 써야 한다. 아래 표의 나머지 줄은 이 사람을
본 적 없는 이전 세대 모델 기준이라, 서로 다른 조건의 값이 한 표에 섞여 있다.
...
겨냥한 조건이다. 얼굴은 빼면 크게 나빠지므로 기여가 크고, 결측 손 채움은 세 모델
세대에서 일관되게 유해했다.

시간축 스무딩(창 7)은 이전 세대에서 +2.1%p였지만 AR 관례를 맞춘 뒤 이득이 대부분
사라졌고, 현재 모델에서는 오히려 −0.7%p다 — flip TTA가 이미 앙상블 역할을 한다.
손 검출률로 거르는 게이트도 같은 커버리지의 confidence 임계보다 약했다(31.4% vs 45.0%).

reject 임계를 올리면 통과분 정확도는 오르고 커버리지는 떨어진다 — 임계 0.4에서
커버리지 58% · 통과분 45.0%다.

### 알려진 구조적 한계

- **한손 재조음 갭**: 어휘 300단어 중 대부분이 학습 데이터에서 양손 조음이다.
  한 손만 보이는 입력(다른 손이 폰을 쥔 상황)이 학습에 없는 분포라는 것이 오랜 병목이었다.
  현재 모델이 **학습 자체를 한 손 입력으로** 돌려 이 지점을 정면으로 다뤘고, 라벨된
  라이브 셋에서 실제로 개선이 확인됐다(아래). 다만 절대 정확도는 여전히 낮다
- **신규 사용자 일반화**: 도메인 적응에 쓴 3명과 다른 사용자에서는 성능이 크게 떨어진다
  (학습 레포 4-fold LOPO 평균 top-1 54.19%). 사용자별 등록 없이 쓰는 것이 현재 조건이다
