...
| `POST /api/v1/sign-sequence` | 문장 → 단어 분해 → 아바타 스켈레톤 시퀀스 (청인 트랙) |
...
→ 음성, 청인→농인은 STT → 단어 분해 → 아바타 스켈레톤 재생이 실제로 돈다.
...
| 청인 트랙 아바타 (`/sign-sequence` + 스켈레톤 재생) | 코드 완료 — **자산이 41단어뿐**이라 300단어 어휘를 못 덮는다. 조음 정확성 육안 검증 전 |
