...
- 캡처 햅틱 — **신호는 「기록 시작」 하나뿐이다.** 시작/종료/확정 세 신호를 횟수로 구분하던
  설계는 실기기에서 진동이 불규칙한 문제로 되돌렸다(2026-08-26) — 원인이 안 울리는 것이
  아니라 신호끼리 서로를 잘라 먹는 것이었고, 신호가 하나면 겹칠 대상이 없다. 근거와 무엇을
  잃었는지는 `capture/haptics.ts` 「계약」에 있다. 진동 길이(`PULSE_MS` 200ms)는 임시값이고
  약하다는 피드백으로 45→90→200 으로 올렸다. ⚠️ **세기는 노브가 없다** — Vibration API 에
  진폭 파라미터가 없어 길이로만 체감 세기를 바꾼다. 200ms 로도 체감이 같다면 원인은 길이가
  아니다(아이폰이면 조절 자체가 불가능하다)
- 개발 화면(`?dev=1`)에 **햅틱 진단 패널**이 있다 — 경로·`vibrate()` 반환값·번들 상수·신호
  집계를 폰 화면에서 읽는다. "브라우저가 거부"와 "기기가 안 울림"을 가르는 자리다
