...
종횡비(AR) 보정 — 정규화 이전 입력측 사영:
  (a) MediaPipe 정규화 좌표(x/너비, y/높이)는 **프레임 종횡비에 따라 이방성이 달라진다.**
      그래서 "어떤 종횡비의 정규화 좌표인가"가 모델 입력 계약의 일부다. 보정식
      **x' = x × (AR_live / AR_TRAIN)** (y 불변)은 라이브 좌표를 "같은 장면을 AR_TRAIN
      프레임으로 찍었을 때의 정규화 좌표"로 사영한다. AR_live == AR_TRAIN 이면 항등이다.
  (b) ⚠️ **AR_TRAIN 은 모델마다 다르다.** 서빙 모델(Single-Observed-Hand)은 폰 세로
      촬영본으로 도메인 적응을 했고 학습 쪽 전처리에 AR 보정이 없다 — 즉 **세로 정규화
      좌표를 그대로 기대한다.** 그래서 AR_TRAIN 이 9/16 이고, 세로 캡처에는 항등이다.
      (스튜디오 16:9 만으로 학습한 이전 세대는 16/9 였다. 그 값을 이 모델에 쓰면
      x 가 약 1/3 로 찌그러져 들어간다 — 아래 실측.)
  (c) 실증 근거 (2026-08-24, 라벨된 라이브 클립 138개·폰 세로):
        AR_TRAIN 16/9 (x 를 0.316 배로 찌그러뜨림)  top-1 29.0% / top-4 54.3%
        AR_TRAIN 9/16 (항등)                        top-1 39.9% / top-4 69.6%
      같은 조작을 스튜디오 전용 학습 모델(SPOTER-208)에 적용하면 반대로 23.2% → 7.2%
      로 붕괴한다 — 이 값이 **모델의 학습 좌표 관례**라는 근거다.
  (d) ⚠️ **가로 입력은 미검증이다.** 평가 클립이 전량 세로라 "AR_TRAIN=9/16"과
      "보정 자체를 끔"이 이 데이터에서 구분되지 않는다. 도메인 적응에 AI Hub 16:9 도
      섞였으므로 모델이 두 관례 모두에 robust 할 가능성도 남아 있다. 가로 촬영분을
      확보하면 재검토할 것.
...
# 서빙 모델이 기대하는 정규화 좌표 관례의 프레임 종횡비 (모듈 docstring AR 보정 절).
# ⚠️ **모델마다 다른 값이다.** 현재 모델은 폰 세로 촬영본으로 적응했으므로 9/16 이고,
# 세로 캡처에는 배율 1.0(항등)이 된다. 스튜디오 16:9 로만 학습한 모델로 바꾼다면
# 이 값도 16/9 로 함께 되돌려야 한다 — 안 되돌리면 에러 없이 정확도만 무너진다.
AR_TRAIN = 9.0 / 16.0
