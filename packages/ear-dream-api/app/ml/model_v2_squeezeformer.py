"""[미사용 보존] v2 SqueezeformerLite 30단어 서빙 로더 — SPOTER-208 전환(develop 실험
브랜치)으로 서빙 경로에서 제외됐다. app/ml/model.py 가 현재 서빙 로더다.

⚠️ 이 모듈은 어떤 라우트도 import 하지 않는다. v2 복귀 시 참조용으로만 남긴다.
   config 의 구 설정(model_checkpoint_path 등)이 제거돼 이 모듈의 resolve_* 는
   현재 설정으로는 동작하지 않는다 (master 브랜치가 원본이다).
