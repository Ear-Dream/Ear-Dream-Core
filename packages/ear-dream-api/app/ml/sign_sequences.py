...
"어떤 단어에 시퀀스가 있는지" 를 알아야 한다. 현재 어휘 300 중 41단어만 시퀀스가 있다.

⚠️ **매니페스트(커밋됨)와 앱 자산(커밋 안 됨)은 어긋날 수 있다.** 앱 쪽 index.json 을
재생성하지 않은 채 매니페스트만 갱신되면 서버는 "재생 가능" 이라 답하는데 클라이언트에
파일이 없는 상태가 된다. 양쪽 `bundle_version` 을 비교해 감지한다 — 응답
