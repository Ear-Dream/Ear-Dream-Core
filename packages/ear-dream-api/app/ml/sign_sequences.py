...
**좌표는 여기 없다.** 좌표는 클라이언트 빌트인 시퀀스(`packages/ear-dream-app/public/
...
어휘가 시퀀스보다 먼저 늘어나는 것이 정상적인 순서라 이 구분은 남는다.

⚠️ **매니페스트와 앱 시퀀스는 어긋날 수 있다.** 둘 다 커밋되지만 생성 스크립트가 한 번에
...
# 클라이언트가 시퀀스를 찾아가는 기준 경로 (앱 public/ 하위 디렉토리명).
# 응답에 실어 서버·앱 어느 쪽이 경로를 바꿔도 계약으로 드러나게 한다.
SEQUENCE_PATH: str = str(_DATA["sequence_path"])
...
class SequenceEntry:
    word_id: str
    # 클라이언트가 빌트인 시퀀스를 찾아가는 키. 현재 값은 word_id 와 같지만 **별도 필드로
    # 유지한다** — 나중에 한 단어에 조음 변형(variant)이 여러 개 생기거나 시퀀스 이름
