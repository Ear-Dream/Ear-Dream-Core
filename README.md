...
- 청인 트랙: 말하면 브라우저 음성 인식으로 텍스트가 되고, 서버가 문장을 어휘 단어로
  쪼개면 단어별 수어 동작을 스켈레톤으로 재생한다. 음성 인식을 쓸 수 없는 환경에서는
  키보드 입력으로 같은 흐름을 탄다.
...
| `POST /api/v1/sign-sequence` | 동작 — 문장 → 아바타가 재생할 단어 열. 문장 분해는 아직 규칙 mock |
...
| M4 | 음성 → 수어 영상 | 부분 — STT·문장 분해·스켈레톤 재생은 됨. 어휘 300 중 **41단어만** 동작 자산 보유 |
