...
- 농인 → 청인: 수어 동작을 단어 단위로 인식 → 후보 중 선택 → 단어 누적 → 문장 변환 → 텍스트 전달
...
- 수어 트랙: 단어당 버튼을 누르는 동안 동작을 캡처 → 후보 중 선택 → 단어 칩 누적 →
  문장 변환 → 결과 화면. 어휘는 현재 30단어다.
...
| M3 | 오인식 정정, 상황 문장 호출 | 부분 — 후보 전환은 됨, 상황 문장 미착수 |
